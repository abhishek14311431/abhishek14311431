# Abhishek S

### AI & Machine Learning Engineer | B.Tech Student at Lovely Professional University

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/abhishek18)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/abhishek14311431)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:abhishekyadavv4567@gmail.com)

---

## About Me

I am an AI and Machine Learning engineering student with a strong foundation in developing intelligent systems and data-driven solutions. My experience spans from hands-on machine learning projects to practical engineering work, combining technical expertise with real-world problem-solving skills.

- 🎓 Pursuing B.Tech in Artificial Intelligence and Machine Learning at Lovely Professional University
- 🔬 Focused on building scalable AI applications and exploring advanced ML techniques
- 💼 Former On-Site Engineer Intern at Mantha Architects (awarded "Best Performer")
- 📍 Based in Phagwara, Punjab, India
- 📧 Contact: abhishekyadavv4567@gmail.com

---

## Technical Skills

**Programming Languages**  
Python • R • C++ • Java

**AI/ML Frameworks & Libraries**  
NumPy • Pandas • Scikit-learn • TensorFlow • Matplotlib

**Development Tools**  
Git & GitHub • VS Code • MongoDB • SQL

**Web Technologies**  
React • Node.js • Express

**Core Competencies**  
Machine Learning • Data Analysis • Algorithm Design • Problem Solving • Statistical Analysis

---

## Professional Experience

**On-Site Engineer Intern** | Mantha Architects  
*June 2025 – August 2025*

- Managed complete residential project lifecycle from initial planning through final handover
- Coordinated cross-functional teams including contractors and on-site personnel to maintain quality standards
- Utilized AutoCAD, Revit, and SketchUp for technical design execution and project documentation
- Supervised daily site operations while providing technical guidance to team members
- Recognized as "Best Performer" for exceptional project delivery and team coordination

---

## Projects

### AI-Assisted Residential Planning System
*August 2025*

Developed a backend system that automatically generates residential floor plans using rule-based architectural logic and real-world constraints.

**Key Accomplishments:**
- Implemented site scaling algorithms with multi-floor stacking and staircase generation
- Designed wall generation system using real-world architectural units and measurements
- Created automated 2D SVG blueprint generation and 3D building model visualization
- Achieved consistent layout accuracy through enforcement of architectural zoning rules

**Technologies:** Python, Matplotlib, SVG, Object-Oriented Programming, Rule-Based Systems

### Air & Water Quality Monitoring System
*March 2024*

Built a comprehensive environmental monitoring system for real-time analysis of air and water quality parameters.

**Key Accomplishments:**
- Developed data collection pipeline integrating multiple datasets and APIs
- Implemented analysis algorithms for environmental parameters (PM2.5, AQI, pH, turbidity)
- Applied rule-based classification techniques achieving approximately 80% accuracy
- Created interactive dashboard with automated alert system and location-based analytics

**Technologies:** Python, Streamlit/Flask, Pandas, Data Analysis, Database Management

---

## Certifications

- **AI Foundations Associate** – Oracle (October 2025)
- **Python with Data Structures & Algorithms** – CSE Patashala (July 2025)
- **Python Crash Course** – Coursera (February 2023)

---

## Achievements

- 🏆 Best Performer Award – Mantha Architects Internship (September 2025)
- 🎖️ Certificate of Excellence – Online Competitive Programming Challenges (November 2024)
- 📊 Completed 100+ hours of hands-on Machine Learning and Data Science projects (July 2024)
- 🥇 First Place – Inter-College Aptitude Competition (January 2023)

---

## Education

**Bachelor of Technology in Artificial Intelligence and Machine Learning**  
Lovely Professional University, Phagwara, Punjab  
*2023 – Present* | CGPA: 6.73

**Pre-University Course (Science Stream)**  
Expert PU College, Mangalore, Karnataka  
*2023* | Percentage: 86.33%

**Secondary School Certificate**  
Jain Public School, Chintamani, Karnataka  
*2021* | Percentage: 79.83%

---

## GitHub Statistics

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=abhishek14311431&show_icons=true&theme=default&hide_border=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=abhishek14311431&layout=compact&theme=default&hide_border=true)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=abhishek14311431&theme=default&hide_border=true)

</div>

---

## Connect With Me

I'm open to collaboration opportunities, research discussions, and professional networking in the field of AI and Machine Learning.

- **LinkedIn:** [linkedin.com/in/abhishek18](https://linkedin.com/in/abhishek18)
- **Email:** abhishekyadavv4567@gmail.com
- **Phone:** +91-8867142526

---

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=abhishek14311431&color=blue&style=flat)

</div>
